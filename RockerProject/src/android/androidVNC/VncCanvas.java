...
public class VncCanvas extends ImageView {
	...
	void initializeVncCanvas(ConnectionBean bean, final Runnable setModes) {
		...
		Thread t = new Thread() {
			public void run() {
				try {
					connectAndAuthenticate(connection.getUserName(),connection.getPassword());//主要连接方法
					doProtocolInitialisation(display.getWidth(), display.getHeight());//显示桌面初始化代码
					handler.post(new Runnable() {
						public void run() {
							pd.setMessage("Downloading first frame.\nPlease wait...");
						}
					});
					processNormalProtocol(getContext(), pd, setModes);//显示桌面主要代码
				} catch (Throwable e) {
