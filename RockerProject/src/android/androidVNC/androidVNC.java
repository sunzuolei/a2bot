...
import com.rp.DirectionMonitor;
import com.rp.MyConnect;
...
public class androidVNC extends Activity {
	...
	public void onCreate(Bundle icicle) {
		...
		colorSpinner = (Spinner)findViewById(R.id.colorformat);//选择色彩
		...
		groupForceFullScreen = (RadioGroup)findViewById(R.id.groupForceFullScreen);//强制全屏按钮
		checkboxKeepPassword = (CheckBox)findViewById(R.id.checkboxKeepPassword);//保存密码按钮
		checkboxLocalCursor = (CheckBox)findViewById(R.id.checkboxUseLocalCursor);//本地鼠标点击按钮
		colorSpinner.setAdapter(colorSpinnerAdapter);
		colorSpinner.setSelection(0);
		spinnerConnection = (Spinner)findViewById(R.id.spinnerConnection);//选择连接
		...
		repeaterText = (TextView)findViewById(R.id.textRepeaterId);//显示了norepeater这行字
	...
	private void vnc() {
		updateSelectedFromView();//更新连接表单视图
		saveAndWriteRecent();//写到手机数据库中
